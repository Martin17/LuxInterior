 ...
 */
class Result extends Base implements \Iterator, \ArrayAccess, \Countable {
    ...
    protected function quote($val) {
        ...
        if (is_int($val) || $val instanceof \NotORM\Literal) { // number or SQL code - for example "NOW()"
            return (string) $val;
        }
    ...
    function insert($data) {
        ...
        $parameters = array();
        if ($data instanceof \NotORM\Result) {
        ...
        if (is_array($data)) {
            ...
            foreach (func_get_args() as $value) {
                ...
                foreach ($value as $val) {
                    if ($val instanceof \NotORM\Literal && $val->parameters) {
    ...
    function where($condition, $parameters = array()) {
        ...
        if ($args != 2 || strpbrk($condition, "?:")) { // where("column < ? OR column > ?", array(1, 2))
            ...
            $condition .= " IS NULL";
        } elseif ($parameters instanceof \NotORM\Result) { // where("column", $db->$table())
            $clone = clone $parameters;
            if (!$clone->select) {
                $clone->select($this->notORM->structure->getPrimary($clone->table));
            }
            if ($this->notORM->driver != "mysql") {
                if ($clone instanceof \NotORM\MultiResult) {
                ...
                foreach ($clone as $row) {
                    ...
                    if ($clone instanceof \NotORM\MultiResult && count($row) > 1) {
                        array_shift($row);
                    }
                    if (count($row) == 1) {
                        $in[] = $this->quote($row[0]);
     ...
     * @return NotORM_Result fluent interface
     */
    function union(\NotORM\Result $result, $all = false) {
    ...
    protected function execute() {
        if (!isset($this->rows)) {
            ...
            foreach (array_merge($this->select, array($this, $this->group, $this->having), $this->order, $this->unionOrder) as $val) {
                if (($val instanceof \NotORM\Literal || $val instanceof self) && $val->parameters) {
                    $parameters = array_merge($parameters, $val->parameters);
                }
            }
            try {
